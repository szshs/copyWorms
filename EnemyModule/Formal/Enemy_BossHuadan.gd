...
const HOVER_EXTRA_HEIGHT: float = 550.0     # 悬停额外高度（在跳跃顶点基础上再上移）
...
func _spawn_hover_minions() -> void:
	...
	for i in count:
		...
		# 在 Boss 周围随机位置生成（左右各 200-500px，落地高度）
		var side = 1.0 if randf() > 0.5 else -1.0
		var offset_x = side * randf_range(200.0, 500.0)
		var spawn_pos = global_position + Vector2(offset_x, 0)
		# 确保 Y 在地面附近（Boss 当前 Y 即视为地面高度）
