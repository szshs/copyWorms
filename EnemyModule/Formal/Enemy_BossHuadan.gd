...
const MELEE_HIT_FRAME: int = 11          # 第 11 帧出伤
const MELEE_TOTAL_FRAMES: int = 16      # 攻击动画总帧数
const MELEE_HITBOX_DURATION: float = 0.12  # 命中盒持续时间
...
func _physics_process(delta: float) -> void:
	...
	# 近战攻击计时与命中窗口（第 11 帧出伤）
	if _melee_active:
		_melee_elapsed += delta
		var hit_time: float = MELEE_HIT_FRAME / ATTACK_FPS       # ≈0.917s
		...
		if _melee_elapsed >= hit_time and not _melee_hit_done:
			...
			_activate_melee_hitbox(false)
		# hitbox激活期间每帧主动检测重叠（解决玩家已在范围内时body_entered不触发的问题）
