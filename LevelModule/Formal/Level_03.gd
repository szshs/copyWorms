...
func _spawn_lingnan_enemies() -> void:
	if not _enemy_slime_scene:
		push_warning("[Level_03] Enemy_Slime.tscn 缺失")
		return
	...
	for i in range(mini(spawn_points.size(), count)):
		var enemy = _spawn_enemy_with_config(_enemy_slime_scene, spawn_points[i], cleaner_config)
		if enemy:
...
func _spawn_cyber_enemies() -> void:
	if not _enemy_slime_scene: return
