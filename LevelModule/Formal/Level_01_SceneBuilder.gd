...
func _build_terrain() -> void:
	...
	# 主地面：1592 像素宽，顶部 Y=640
	var ground = level._create_static_body("MainGround", Vector2(796, 660), Vector2(1592, 40))
	terrain.add_child(ground)
	# 左/右墙
	var left_wall = level._create_static_body("LeftWall", Vector2(-10, 360), Vector2(20, 720))
	terrain.add_child(left_wall)
	var right_wall = level._create_static_body("RightWall", Vector2(1602, 360), Vector2(20, 720))
...
func _build_interactives() -> void:
	...
	# --- 客厅 (50-550) ---
	# 1. 纸箱阻挡
	level._obstacle_box = level._create_interactive("Obstacle_Box", "box", Vector2(200, 600), Vector2(120, 80))
	level._add_physics_blocker(level._obstacle_box, Vector2(120, 80))
	container.add_child(level._obstacle_box)

	# --- 走廊 (550-1000) ---
	# 2. 脏衣阻挡
	level._obstacle_clothes = level._create_interactive("Obstacle_Clothes", "clothes", Vector2(624, 616), Vector2(100, 80))
	level._add_physics_blocker(level._obstacle_clothes, Vector2(100, 80))
	container.add_child(level._obstacle_clothes)

	# --- 卧室 (1000-1900) ---
	# 3. 休学告知书
	level._notice_node = level._create_interactive("Notice", "notice", Vector2(960, 640), Vector2(40, 40))
	...
	level._computer_node = level._create_interactive("Computer", "computer", Vector2(1056, 576), Vector2(100, 80))
	level._computer_node.is_active = false
	container.add_child(level._computer_node)

	# 6. 手机（初始禁用，IDE崩溃后启用）
	level._phone_node = level._create_interactive("Phone", "phone", Vector2(488, 640), Vector2(50, 40))
	level._phone_node.is_active = false
	container.add_child(level._phone_node)

	# 7. 床 - 允许重复交互
	level._bed_node = level._create_interactive("Bed", "bed", Vector2(1504, 616), Vector2(160, 60))
